...
- **`turbo_overlay_hint` no-ops on non-hintable requests.** The capture block is skipped on regular page navigations, form submissions, and anything that isn't a Turbo prefetch or `:hint` variant fetch. Detection uses `Sec-Purpose: prefetch` (W3C standard, what Turbo sends) plus `X-Turbo-Overlay: hint` for the explicit variant fetch path. Adds `turbo_overlay_prefetch_request?` and `turbo_overlay_hintable_request?` controller predicates.
