module TurboOverlay
  # Default CSS for the gem's shipped layouts. Emitted once into
  # the host page's <head> via the `turbo_overlay_styles` view
  # helper — see `lib/turbo_overlay/helpers/view_helper.rb`.
  #
  # Users who want their own styles can skip the helper and provide
  # equivalent rules themselves; the class names are deliberately
  # short and stable. Users who want to tweak something (a max
  # width, a transition timing) can keep the helper and add their
  # own CSS later in the cascade.
  module Styles
    DEFAULT = <<~CSS.freeze
      ...
      dialog.turbo-drawer::backdrop { background: rgba(0, 0, 0, 0.5); }
      dialog.turbo-drawer.turbo-overlay--drawer-right  { inset: 0 0 0 auto; height: 100vh; width: 24rem; }
      dialog.turbo-drawer.turbo-overlay--drawer-left   { inset: 0 auto 0 0; height: 100vh; width: 24rem; }
      dialog.turbo-drawer.turbo-overlay--drawer-top    { inset: 0 0 auto 0; width: 100vw; max-height: 50vh; }
      dialog.turbo-drawer.turbo-overlay--drawer-bottom { inset: auto 0 0 0; width: 100vw; max-height: 50vh; }
      .turbo-drawer__content { display: flex; flex-direction: column; height: 100%; }
      ...
      @media (prefers-reduced-motion: reduce) {
        ...
        dialog.turbo-overlay.turbo-overlay-closing::backdrop { animation: none !important; }
      }

      /* === Scroll lock when any overlay is open === */
      html:has(dialog.turbo-overlay[open]) { overflow: hidden; }
    CSS
  end
end
